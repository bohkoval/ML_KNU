...
An overall pseudo-random number generator seed was set at 5432 for all code. In order to reproduce the results below, the same seed should be used. Different packages were downloaded and installed, such as caret and randomForest. These should also be installed in order to reproduce the results below (please see code below for ways and syntax to do so).
...
For Random Forests we use the following formula, which yielded a much better prediction in in-sample. Writing the predictions to `prediction.csv` file:

```{r}
predictionsB2 <- predict(modFitB1, testing, type = "class")

originalTesting$classe <- predictionsB2
predictions <- subset(originalTesting[,c(ncol(originalTesting),1:(ncol(originalTesting)-1))], select = -c(X))
